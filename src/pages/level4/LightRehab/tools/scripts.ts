 ...
 * show QR code
 */
export function readPeriod(): number {
  /* TO DO */
  return 91;
}
...
export function needMRIPlan(): boolean {
  ...
  if (noLabTest(ltData) && ccData != undefined && mhData != undefined) {
    const painReasons = mhData.list1 ?? []
    const hasSport = painReasons?.[2]?.selected
    const hasAccident = painReasons?.[3]?.selected
    if (ccData.pain_nature_idx === 0 || hasSport || hasAccident) {
      return true;
    }
  }
  /**
