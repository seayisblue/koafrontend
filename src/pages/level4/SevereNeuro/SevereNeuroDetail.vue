...
onActivated(() => {
  const titleKey = `severe-neuro.detail-title-${exerciseId.value}`
...
<template>
  <div class="page-wrapper">
    <div class="snd-container">
      ...
      <div class="snd-title">
        {{ t(`severe-neuro.detail-title-${exerciseId}`) }}
      </div>
      ...
      <div class="snd-sections">
        ...
        <div v-if="exerciseConfig.hasPrepare" class="snd-card">
          <div class="snd-card-title">
            {{ t("severe-neuro.prepare-title") }}
          </div>
          <div class="snd-card-content">
            {{ t(`severe-neuro.prepare-${exerciseId}`) }}
          </div>
        </div>
        ...
        <div class="info-row" v-if="exerciseConfig.hasFrequency || exerciseConfig.hasDuration">
          ...
          <div v-if="exerciseConfig.hasFrequency" class="snd-card half-card">
            <div class="snd-card-title">
              {{ t("severe-neuro.frequency-title") }}
            </div>
            <div class="snd-card-content center-content">
              {{ t(`severe-neuro.frequency-${exerciseId}`) }}
            </div>
          </div>

          <div v-if="exerciseConfig.hasIntensity" class="snd-card half-card">
            <div class="snd-card-title">
              {{ t("severe-neuro.intensity-title") }}
            </div>
            <div class="snd-card-content center-content">
               {{ t(`severe-neuro.intensity-${exerciseId}`) }}
            </div>
          </div>
        </div>
        ...
        <div v-if="exerciseConfig.hasSteps" class="snd-card">
          <div class="snd-card-title">
            {{ t("severe-neuro.steps-title") }}
          </div>
          <div class="snd-card-content">
            {{ t(`severe-neuro.steps-${exerciseId}`) }}
          </div>
        </div>
      ...
      <div class="bottom-tips-area">
        ...
        <div v-if="exerciseConfig.hasTip" class="tips-content">
           <div class="tips-title">{{ t("severe-neuro.detail-tip-title") || "小提示" }}</div>
           <div class="tips-text">{{ t(`severe-neuro.detail-tip-${exerciseId}`) }}</div>
        </div>
      </div>
