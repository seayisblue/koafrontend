...
const exerciseVideoMap: Record<string, FileKey2> = {
  'stairs-single': 'walk_up_and_down_the_block',
  'stand-to-sit': 'stand_to_sit',
  'stand-on-one-leg': 'stand_on_one_leg',
  'walk-straight': 'walk_straight',
  'mini-squat': 'mini_squat',
  'quad-stretch': 'quadriceps_stretching',
  'slide-forward-and-back-on-the-ground': 'slide_forward_and_back_on_the_ground',
}
...
const stepsData = computed(() => {
  const nodes = planList.value.map((id, index) => {
    ...
    return {
      label: t(`moderate-neuro.exercise-${id}`),
...
watch(() => exerciseId.value, (newId) => {
  ...
  const titleKey = `moderate-neuro.exercise-${newId}`
